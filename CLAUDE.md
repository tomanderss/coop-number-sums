...
| `js/shopitems.js` | **Generisches Shop-Framework** für alle weiteren Kategorien: `SHOP_CATS` (palette/theme/sfx/font/frame/badge/skinpreset, je `settingKey` + Gratis-Default; skinpreset = Anwenden-Semantik ohne settingKey, setzt Besitz des exklusiven `dynamicColor`-Skins voraus), `SHOP_CATALOG` (global eindeutige IDs, per Test erzwungen), `SHOP_TIER_PRICES` 400/600/900/1500, `shopInvKey` = `<cat>_<id>` im Union-Inventar, `resolveEquipped` (Fallback auf Gratis-Default), `applyPaletteFx` (reine Hue-Rotation + geklemmte S/L — erhält Farbabstände), `badgeIcon` (sicherer Lookup, fremde RTDB-Strings nie roh rendern). Rendering/Kauf/Equip in app.js (`buyShopItem`/`equipShopItem`/`applySkinPreset`); **Live-Vorschau für jedes Item** (`state.shopPreview`, ▶ je Karte): Demo-Brett für palette/font/frame/skinpreset, Namens-Chip für Badges, Themes 4 s echt (`applyTheme(previewId)`), Hör-Demo für sfx. Badges laufen durch IDENTITY/Präsenz/Bestenliste mit. |
