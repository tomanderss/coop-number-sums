...
| `js/wineffects.js` | Katalog + reine Logik der **kaufbaren Sieganimationen** (erste echte Shop-Kategorie): 23 Effekte (`confetti` gratis + 22 kaufbar, Preisstaffel 400/600/900 via `TIER_PRICES`), Inventar-Keys `winfx_<id>` (`winEffectInvKey`), Besitz/Fallback (`ownsEffect`/`resolveActiveEffect` — nicht Besessenes fällt auf Confetti zurück). Unit-getestet. Partikel-Erzeugung: `PIECE_GENERATORS`/`launchWinFx` in app.js (ein globales fixed Overlay `.winfx.fx-<id>`, auch für die Shop-Vorschau auf jedem Screen); Optik komplett in styles.css — **nur transform/opacity animieren** (GPU-Compositor, Lehre aus dem iOS-Skin-Crash). Kauf: `spendCurrency` + `grantInventory` (union-synct via Cloud), Auswahl im Shop UND unter Einstellungen ▸ Darstellung. |
